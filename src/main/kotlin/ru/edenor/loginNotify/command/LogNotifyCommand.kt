...
object LogNotifyCommand {
    ...
    fun command(): LiteralCommandNode<CommandSourceStack> {
        val rootCommand = literal<CommandSourceStack>("lognotify")
            .requires {
                it.sender.hasPermission(listPermission) || it.sender.hasPermission(editPermission)
            }
            .executes {
            ...
            .then(
                literal<CommandSourceStack>("add")
                    .requires { it.sender.hasPermission(editPermission) }
                    .then(
            ...
            .then(
                literal<CommandSourceStack>("remove")
                    .requires { it.sender.hasPermission(editPermission) }
                    .then(
                        argument<CommandSourceStack, String>("name", StringArgumentType.word())
                            .executes {
            ...
            .then(
                literal<CommandSourceStack>("list")
                    .requires { it.sender.hasPermission(listPermission) }
                    .executes {
                        sendList(it.source.sender)
                        Command.SINGLE_SUCCESS
                    }
            )
    ...
    fun sendHelp(sender: CommandSender) {
        // Здесь ты можешь отправить описание доступных команд
        sender.sendMessage("§aИспользование: /lognotify [add|remove|list]")
    }

    fun sendList(sender: CommandSender) {
        // Здесь логика для вывода списка
        sender.sendMessage("§eСписок пользователей пока не реализован.")
    }
}
*/
