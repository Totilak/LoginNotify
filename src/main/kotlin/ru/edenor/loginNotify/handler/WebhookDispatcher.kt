...
class WebhookDispatcher(val httpClient: HttpClient) {
  ...
  fun toRequest(record: NotificationRecord): DiscordWebhookRequest {
    val embedAuthor = EmbedAuthor(LoginNotify::class.java.simpleName)
    embedAuthor.iconUrl = "https://crafthead.net/helm/${record.playerName}"
    ...
    embed.fields =
        listOf(
            EmbedField("Comment:", record.comment),
            EmbedField("Добавлен в", record.createdAt.toLoginNotifyFormat()))
    val request = DiscordWebhookRequest()

    request.avatarUrl =
        if (record.addedBy == "WesOrangePower")
            "https://crafthead.net/avatar/WesOrangePower"
        else "https://crafthead.net/helm/${record.addedBy}"
